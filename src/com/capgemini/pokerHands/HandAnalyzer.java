...
public class HandAnalyzer {
    ...
    // REVIEW bzychal - what's the meaning of this name? It doesn't match to the above scheme, ZERO_VALUE or smth would
    // be better
    ...
    // REVIEW bzychal - the cases in this method are exclusive, it is unnecessary to execute all of them, first match
    // should return value
    public static Integer getScoreForHand(Hand hand) {
        return getScoreForFlush(hand) + getScoreForStraight(hand) + getScoreForFour(hand) + getScoreForFullHouse(
                hand) + getScoreForThreeOfKind(hand) + getScoreForTwoPairs(hand) + getScoreForOnePair(hand);
    }
    ...
    private static boolean isFullHouse(Hand hand) {
        ...
        return listAppearance.size() == TWO_CARD_THE_SAME_VALUE && listAppearance.get(0)
                .getValue() == THREE_CARD_THE_SAME_VALUE;
    }
    ...
    private static boolean isStraight(Hand hand) {
        return (hand.getCardList().getFirst().getValue() - hand.getCardList().getLast()
                .getValue() == DIFFERENCE_BEETWEEN_VALUE_FIRST_CARD_AND_VALUE_LAST_CARD_IS_FOUR) && (hand
                        .getListAppearanceCardValue().size() == FIVE_DIFFERENT_VALUE_CARD);
    }
    ...
    private static boolean isThreeOfKind(Hand hand) {
        return hand.getListAppearanceCardValue().get(0).getValue().equals(
                THREE_CARD_THE_SAME_VALUE) && hand.getListAppearanceCardValue().size() == THREE_DIFFERENT_VALUE_CARD;
    }
    ...
    private static boolean isTwoPairs(Hand hand) {
        return hand.getListAppearanceCardValue().get(0).getValue().equals(TWO_CARD_THE_SAME_VALUE) && hand
                .getListAppearanceCardValue().get(1).getValue().equals(TWO_CARD_THE_SAME_VALUE);
    }
